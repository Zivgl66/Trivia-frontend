...
export const createLeaderboard = (leaderboardData) => async (dispatch) => {
  try {
    ...
    // console.log("data leaderboard created: ", data);
    dispatch({ type: CREATE_LEADERBOARD, payload: data.leaderboard });
...
export const updateQuestionLeaderboard = (questionResult, id) => async (
  dispatch
) => {
  try {
    const { data } = await api.updateQuestionLeaderboard(questionResult, id);
    dispatch({ type: UPDATE_QUESTION_LEADERBOARD, payload: data.leaderboard });
    return data.leaderboard;
  } catch (error) {
    console.log(error);
  }
...
export const updateCurrentLeaderboard = (result, id) => async (dispatch) => {
  try {
    ...
    dispatch({ type: UPDATE_CURRENT_LEADERBOARD, payload: data.leaderboard });
    return data.leaderboard;
  } catch (error) {
    console.log(error);
  }
